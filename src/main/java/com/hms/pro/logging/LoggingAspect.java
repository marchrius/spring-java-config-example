...
public class LoggingAspect {
	 ...
	 */
	@After("execution(* com.hms.pro.serviceImpl.*.*(..)) || execution(* com.hms.pro.daoImpl.*.*(..))")
	 ...
	 *            information at execution point
	 */
	@Before("execution(* com.hms.pro.serviceImpl.*.*(..)) || execution(* com.hms.pro.daoImpl.*.*(..))")
	public void logBeforeMethod(JoinPoint jointPoint) {
		StringBuffer logStr = new StringBuffer();

		logStr.append("\nInvoking Before Method: \nClass :" +jointPoint.getThis()+
				"\nMethod Full Signature : "
				+ jointPoint.getSignature().toLongString() + "\nName : "
	 ...
	 */
	@AfterReturning(
	        pointcut="execution(* com.hms.pro.daoImpl.*.*(..))",
	        returning="result")
